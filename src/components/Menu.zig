...
pub fn menu(config: Config, items: anytype) MenuItem {
    return MenuItem{ .config = config, .items = internal.allocator.dupe(MenuItem, &items) catch @panic("OOM ") };
}

/// 'menus' is a tuple
pub fn menuBar(menus: anytype) MenuBar {
    return MenuBar{ .menus = internal.allocator.dupe(MenuItem, &menus) catch @panic("OOM") };
}
